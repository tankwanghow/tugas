...
duty workflow/recurrence, types & membership management, audit, uploads). A post-v1
...
- **Dual UI:** Desktop `/entities/:entity_slug/...`, Mobile `/m/:entity_slug/...`, with an
  `AutoRouteByDevice` plug (mobile-capable tails: `""`, `/duties/new`, `/duty-types`,
  `/todos`, `/todos/new`, plus the `/duties/:id` show + `/invite-session/:role` regexes) + a
  ...
  **There is no separate duty index page** — the **dashboard is the duty list** on both
  ...
  view; both render rows as LiveView **streams** built by `DutyLive.IndexHelpers.load_page/7`
...
- **Shell-Escape contract:** both layout shells (`#tugas-shell`) bind
  ...
  on Escape. `ModalEscape.close_duty_modals/2` is the shared closer for duty pages
...
mix test test/tugas/duties_test.exs            # single file
mix test test/tugas/duties_test.exs:42         # single test by line
...
**Skip** (`Duties.skip/3`, manager/admin) is the unified close-without-completing action: it
...
successor. This action replaced the former `cancel_duty/3` and `skip_cycle/3` functions.

### Duties domain — the core model (read the spec before editing)

The single most important design decision: **one `Duty` row per cycle**, not a standing
series with a rolling `due_by`. A recurrence chain is linked by a shared `series_id` (UUID).

- `Tugas.Duties.Duty` — one cycle. There is **no `status` string column**; terminal state is expressed via timestamps. A cycle is **live** while `completed_at IS NULL AND closed_at IS NULL` — that's the set dashboards show and that can be worked/completed/skipped. Done stamps `completed_at`; Skip stamps `closed_at` (writes a `skipped` event); End series stamps `closed_at` + `series_ended_at` (writes a `series_ended` event). "Who performed" each terminal action is on the terminal event's `status_by_id` — there are no `done_by`/`skipped_by` columns. This liveness predicate is defined **once** as `Duties.live/1` (a composable query builder) and every list/dashboard/report composes it — never hand-write it. A partial unique index on `series_id` (where live) enforces **one live cycle per series**. `series_ended_at` (when set) blocks future spawning. The row also **snapshots** `complete_documents` from the type at creation (see rule 1). `primary_assignee_id` is **nullable** (unassigned duties). **Title is capped at 60 chars** (`validate_length` on the changeset; the UI uses the `char_count_input` component with a live "characters left" counter). The completed-in-error columns — `completed_in_error_at`/`_by_id`/`_reason`, plus self-referential `replaces_id`/`replaced_by_id` FKs — link a wrongly-completed cycle to its replacement (see rule 4).
- `Tugas.Duties.Event` (`duty_events`) — **append-only** status steps shaped `open → in_progress* → done | skipped | series_ended`. New status = new row; rows are never deleted and status is never rewritten. The step `note` lives here (open context, each progress note, done comment, skip/end-series reason). **`open` is singular** (one per cycle, created at creation) and the terminal event singular (created at completion/skip/end-series), but **`in_progress` may repeat** — every *Update progress* appends another logged `in_progress` event. `start_progress`'s guard (`ensure_progressable`) only rejects a cycle whose terminal event exists — i.e. `event.status in Event.terminal_statuses/0` (= `["done","skipped","series_ended"]`) — with `{:error, :not_live}`; it no longer blocks an already-in-progress cycle.
- `Tugas.Duties.EventDocument` — file uploads attached to an event; the per-file column is **`file`** (a `%{filename, original, path}` map), not `documents`. A live file may be hard-**deleted within 48h** (`document_deletable?`); after that (or for admin-on-locked-cycle) it is **voided** (`voided_at`/`voided_by_id`/`void_reason`) — voided files are kept for audit and **remain downloadable**. `document_slot` matches a name in the duty's snapshotted `complete_documents` for Done validation, and is **immutable after upload** — there is no Replace and no slot-editing; to change a slot's file, delete/void it and re-upload (uploading is only offered for an unsatisfied slot). A document is classified **required** when its `document_slot` is in the duty's current snapshot `complete_documents`, otherwise **supporting** (no slot, or a slot no longer in the set).
- `Tugas.Duties.AuditLog` — field-level before/after for **corrections** (title, due_by, assignee, note edits).
- `Tugas.Duties.Type` — **per-entity only** (`entity_id` is **NOT NULL**). There are no
  global system presets; instead, when an entity is created, `Tugas.Duties.SampleTypes`
...
The duty Documents UI is split by what a file is **for**, with each file shown
in exactly one place (no duplication):

- **Completion Documents** (`DutyCompletionDocuments`) — **cycle-level**, one
  modal per duty: a row per required slot (live file inline, or an inline
  ...
  else `open`). The duty summary shows each required slot beside the title with
  ...
  no longer live.
- **Step Files** (`DutyStepFiles`) — **per-step**, a modal per timeline event:
  that step's supporting (no-slot/stale-slot) files + a voided-other section + an
  additional-file uploader.

Classification and partitioning live in `TugasWeb.DutyLive.DocumentHelpers`
...
duties' snapshot only (completed/closed cycles stay frozen); a file whose slot was
...
documents / step files). The old `DutyDocumentUpload`/`DutyDocumentList`
...
(`POST /entities/:entity_slug/duties/:duty_id/documents`, device-agnostic
...
`Tugas.Duties.add_document/5` remains the context entry point.
...
1. **Done validation is enforced only on Done**, never earlier. A Done **note is always
   ...
   duty snapshot). The only **snapshotted** completion rule is `complete_documents` (copied
   from the type at creation), validated against the duty's snapshot, **not the live type**
   ...
   `Duties.Completion`. **Only the completion contract is frozen** — `reminder_offsets`
...
2. **Recurrence on Done.** Done is a **guarded close**: a conditional `update_all` stamps
   ...
   `{:error, :next_due_required}`) and a **new** Duty is spawned with the same `series_id`,
   ...
   The 8 intervals live in `Duties.Recurrence`; fixed intervals pre-fill the next `due_by`
...
3. **Corrections lock once a cycle is no longer live.** While a cycle is live (`completed_at IS NULL AND closed_at IS NULL`): managers/admins edit duty fields; note authors edit their own note within **48 hours** (manager/admin override anytime while live). After the cycle is Done or closed (skipped / series-ended) everything is locked except admin-only void-with-reason. Every correction is logged in `AuditLog`.

4. **Completed-in-error is a stamp, not a revert.** `Duties.mark_completed_in_error/3`
...
- `Duties.Urgency.classify(type, due_by, today)` → `:overdue | :due_soon | :ok`, where
...
- `Duties.Urgency.tier(type, due_by, today)` → `:overdue | :critical | :due_soon | :approaching
...
- **Someday = duties with no due date, surfaced by a sort (not a filter).** `due_by` is **nullable**;
  ...
  filtering; the core `live/1` and the older `list_duties/2` are unchanged). The **`Someday`
...
- **The list is server-paginated, never a full load.** `Duties.list_duties_page/2`
  ...
  `Duties.Pagination` codec) for **every** lifecycle. The one in-memory exception is **`Most
  urgent` + `Live`**: `IndexHelpers` ranks a **1-year `due_by` window** in memory (`Urgency.classify`
  needs the entity-tz `today` + type offsets, impractical in SQL — and ranks bare duties,
...
### Quick Todos — lightweight team tasks (separate from duties)

Todos are a deliberately **simpler, parallel domain** to duties: an entity-scoped, team-visible
...
mirror the duty conventions (scope-first, `Ecto.Multi` writes, `:not_authorise`/`:not_found`
returns, keyset pagination, LiveView streams), and **escalate into** a real duty when needed.

- **`Tugas.Todos.Todo`** — one row per todo (`title` capped at **200** chars). Like duties,
  ...
  **escalated** are their own terminal stamps (each with `_at`/`_by_id`; escalation also stores
  `escalated_duty_id`). Completing is **reversible** (toggle re-opens) — `completed_at`/`_by_id`
  flip on/off.
...
- **Audit trail.** Every mutation writes a `Todos.AuditLog` row (`created`/`updated`/`completed`/
  ...
  and the duty id). Per-todo history is inline-expandable on each row; the **team log** pages
...
- **Routes & UI.** Desktop `/entities/:slug/todos` (`TodoLive.Index`) + `/todos/team-log`; mobile
  ...
  delegation — the same Desktop+Mobile split used for duties. **There is no status filter** —
...
- **Context modules own domain logic.** LiveViews call `Tugas.Duties`, `Tugas.Entities`, `Tugas.Accounts` — not Repo directly.
- **Multi-step writes use `Ecto.Multi`/transactions** (create duty + open event; Done + spawn next; skip + event).
- File uploads (v1) go to the local filesystem under a **configurable** `:uploads_dir`
  (`config :tugas, :uploads_dir`), laid out `:entity_id/:duty_id/`; it defaults to the priv
