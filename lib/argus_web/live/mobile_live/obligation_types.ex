defmodule ArgusWeb.MobileLive.ObligationTypes do
  ...
  def handle_event("clone", %{"id" => id}, socket) do
    source = Obligations.get_type!(socket.assigns.current_scope, id)
  ...
  def handle_event("validate", %{"type" => params}, socket) do
    ...
    changeset = Obligations.change_type(template, params) |> Map.put(:action, :validate)
  ...
  def handle_event("save", %{"type" => params}, socket) do
    ...
    result =
      case socket.assigns.editing do
        nil -> Obligations.create_type(scope, params)
        %Type{} = type -> Obligations.update_type(scope, type, params)
      end
  ...
  defp open_modal(socket, template, editing, title, submit_label) do
    changeset = Obligations.change_type(template)
