defmodule TugasWeb.MobileLive.DutyShow do
        ...
        <section id="duty-summary" class="tugas-workbench tugas-duty-summary">
          ...
          >
            <div class="flex justify-between items-center gap-2">
              ...
              <div :if={@duty.due_by} class="font-medium text-base-content shrink-0">
                <span class="text-warning">Due </span>{format_date(@duty.due_by, :short)}
              </div>
              <div :if={is_nil(@duty.due_by)} class="font-medium text-base-content shrink-0">
            ...
            <div class="flex flex-wrap items-center gap-1.5">
              ...
              >
                ...
                <span class="truncate">{user_label(@duty.primary_assignee)}</span>
          ...
          <div class="flex flex-wrap items-center gap-2">
            <h1 class="font-semibold leading-tight min-w-0 flex-1">{@duty.title}</h1>
            <.cycle_badge
          ...
          <div
            ...
            class={TugasWeb.DutyDocumentThumb.thumb_grid_classes(:mobile)}
          >
          ...
          >
            <div id="duty-progress-actions" class="tugas-inline-actions gap-2">
              <button
            ...
            <div id="duty-done-actions" class="tugas-inline-actions">
              <button
            ...
            <div id="duty-series-actions" class="tugas-inline-actions gap-2">
              <button
          ...
          >
            ...
            <p class="text-base-content/70">{@duty.completed_in_error_reason}</p>
            <.link
              :if={@duty.replaced_by_id}
              navigate={~p"/m/#{@current_scope.entity.slug}/duties/#{@duty.replaced_by_id}"}
          ...
          >
            ...
            <.link
              navigate={~p"/m/#{@current_scope.entity.slug}/duties/#{@duty.replaces_id}"}
              class="link link-primary"
            >
              View original
            </.link>
        ...
        <section class="tugas-section">
          <ol id="event-timeline">
            ...
            >
              ...
              <div id={"m-event-note-#{event.id}"} class="tugas-event-note-block relative">
  ...
  def handle_event("confirm_start_progress", %{"progress" => %{"note" => note}}, socket) do
    case Duties.start_progress(socket.assigns.current_scope, socket.assigns.duty, %{
           note: note
         }) do
      {:ok, _} ->
        {:noreply,
  ...
  def handle_event("complete", %{"done" => params}, socket) do
    ...
    case Duties.complete(scope, socket.assigns.duty, attrs) do
    ...
    {:noreply,
     ...
     |> assign(
       ...
       to_form(
         %{
           "note" => "",
           "next_due_by" => suggestion(duty, socket.assigns.recurring?)
         },
         as: :skip
       )
     )}
  ...
  def handle_event("open_correct_modal", _params, socket) do
    {:noreply,
     ...
     |> assign(:correct_form, correct_form(socket.assigns.duty))}
  end
    ...
    case Duties.mark_completed_in_error(scope, duty, attrs) do
      {:ok, _original, replacement} ->
        {:noreply,
         ...
         |> push_navigate(to: ~p"/m/#{scope.entity.slug}/duties/#{replacement.id}")}
  ...
  defp load(socket, id, scope) do
    ...
    |> assign(
      :done_form,
      to_form(%{"note" => "", "next_due_by" => suggestion(duty, recurring?)}, as: :done)
    )
    |> assign(
      :skip_form,
      to_form(%{"note" => "", "next_due_by" => suggestion(duty, recurring?)}, as: :skip)
    )
    |> assign(:end_series_form, to_form(%{"note" => ""}, as: :end_series))
    |> assign(:correct_form, correct_form(duty))
  ...
  defp reload(socket) do
    scope = socket.assigns.current_scope
    duty_id = socket.assigns.duty.id
    socket = load(socket, duty_id, scope)
    ...
    case socket.assigns.step_files_modal_event_id do
      ...
      event_id ->
        assign(
          ...
          find_event(socket.assigns.duty.events, event_id)
        )
    end
  end
  ...
  defp suggestion(duty, true) do
    case Recurrence.next_due_suggestion(duty.duty_type, duty.due_by) do
      %Date{} = date -> Date.to_iso8601(date)
      _ -> ""
    end
  end

  defp suggestion(_duty, false), do: ""
